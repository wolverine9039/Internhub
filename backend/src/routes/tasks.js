...
router.get('/', authenticate, async (req, res, next) => {
  try {
    ...
    const { sortField, sortOrder } = parseSorting(req.query, ['id', 'title', 'status', 'priority', 'due_date', 'created_at']);
    ...
    const [[countResult], [tasks]] = await Promise.all([
      pool.execute(`SELECT COUNT(*) AS total FROM tasks t ${where}`, params),
      pool.execute(`
    ...
    res.status(200).json(paginatedResponse(tasks, page, pageSize, countResult[0].total));
  } catch (err) { next(err); }
});
...
router.patch('/:id', authenticate, authorize('admin', 'trainer'), async (req, res, next) => {
  try {
    const { setClauses, params } = buildPatchFields(req.body, ['title', 'description', 'status', 'priority', 'due_date', 'assigned_to']);

    params.push(req.params.id);
    const [result] = await pool.execute(`UPDATE tasks SET ${setClauses.join(', ')} WHERE id = ?`, params);
