...
const EvaluationFormModal: React.FC<EvaluationFormModalProps> = ({ isOpen, editEvaluation, onSubmit, onClose }) => {
  ...
  let submitText = 'Create Evaluation';
  if (submitting) {
    submitText = 'Saving...';
  } else if (editEvaluation) {
  ...
  return (
    <div className="modal-overlay" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }} aria-hidden="true">
      <dialog open className="modal-card" style={{ width: '600px', maxWidth: '90vw', border: 'none', padding: 0, margin: 0 }} aria-modal="true" aria-label={editEvaluation ? 'Edit Evaluation' : 'New Evaluation'}>
          ...
          <div className="modal-body" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
            ...
            <div className="two-col">
              <div className="form-group">
                <label className="form-label" htmlFor="eval-submission">Submission</label>
                <select id="eval-submission" className={`form-input ${errors.submission_id ? 'error' : ''}`} value={submissionId} onChange={e => setSubmissionId(e.target.value)} disabled={!!editEvaluation}>
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="eval-trainer">Evaluator (Trainer)</label>
                <select id="eval-trainer" className={`form-input ${errors.trainer_id ? 'error' : ''}`} value={trainerId} onChange={e => setTrainerId(e.target.value)}>
            ...
            <div className="form-group">
              ...
              <div className="two-col" style={{ gap: '10px' }}>
                <div>
                  <label className="form-label" htmlFor="eval-cq">Code Quality ({codeQuality})</label>
                  <input id="eval-cq" type="range" min="1" max="10" value={codeQuality} onChange={e => setCodeQuality(Number(e.target.value))} style={{ width: '100%' }} />
                </div>
                <div>
                  <label className="form-label" htmlFor="eval-fn">Functionality ({functionality})</label>
                  <input id="eval-fn" type="range" min="1" max="10" value={functionality} onChange={e => setFunctionality(Number(e.target.value))} style={{ width: '100%' }} />
                </div>
                <div>
                  <label className="form-label" htmlFor="eval-doc">Documentation ({documentation})</label>
                  <input id="eval-doc" type="range" min="1" max="10" value={documentation} onChange={e => setDocumentation(Number(e.target.value))} style={{ width: '100%' }} />
                </div>
                <div>
                  <label className="form-label" htmlFor="eval-time">Timeliness ({timeliness})</label>
                  <input id="eval-time" type="range" min="1" max="10" value={timeliness} onChange={e => setTimeliness(Number(e.target.value))} style={{ width: '100%' }} />
                </div>
              </div>
            ...
            <div className="form-group">
              <label className="form-label" htmlFor="eval-feedback">Feedback Notes</label>
              <textarea id="eval-feedback" className="form-input" style={{ minHeight: '80px', resize: 'vertical' }} value={feedback} onChange={e => setFeedback(e.target.value)} placeholder="General comments..." />
            </div>

            <div className="two-col">
              <div className="form-group">
                <label className="form-label" htmlFor="eval-strengths">Key Strengths</label>
                <textarea id="eval-strengths" className="form-input" style={{ minHeight: '60px', resize: 'vertical' }} value={strengths} onChange={e => setStrengths(e.target.value)} placeholder="e.g. Good variable naming" />
              </div>
              <div className="form-group">
                <label className="form-label" htmlFor="eval-improvements">Areas for Improvement</label>
                <textarea id="eval-improvements" className="form-input" style={{ minHeight: '60px', resize: 'vertical' }} value={improvements} onChange={e => setImprovements(e.target.value)} placeholder="e.g. Needs more comments" />
              </div>
            </div>
          </div>
